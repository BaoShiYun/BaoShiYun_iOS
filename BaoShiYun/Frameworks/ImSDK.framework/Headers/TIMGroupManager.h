 ...
 *  获取自己所加入的群列表。
 *
 *  @param succ 成功回调，NSArray 列表为 TIMGroupInfo
 *  @param fail 失败回调
 *
 *  @return 0：成功；1：失败
 *
 *  @note 该接口有频限检测，SDK 限制调用频率为 1 秒 10 次，超过限制后会报 ERR_SDK_COMM_API_CALL_FREQUENCY_LIMIT （7008）错误
 */
 ...
 *
 *  @param groups 需要拉取详细信息的群组Id列表
 *  @param succ 成功回调，NSArray 列表为 TIMGroupInfoResult
 *  @param fail 失败回调
 *
 *  @return 0：成功；1：失败
 */
...
*  @note 该接口有频限检测，SDK 限制调用频率为60秒1次
*/
- (int)getGroupOnlineMemberCount:(NSString*)groupId succ:(TIMGetGroupOnlineMemberCountSucc)succ fail:(TIMFail)fail;
 ...
 *  5.5 修改加群选项
 *
 *  1. 公开群、聊天室：只有群主或者管理员可以修改加群选项。
 *  2. 音视频聊天室：不能设置加群选项，成员默认可以随意进入。
