 ...
 *  @note 该接口待废弃，删除消息请调用 TIMConversation -> deleteMessages 接口，此接口支持服务器删除消息。
 */
- (BOOL)remove DEPRECATED_ATTRIBUTE;
 ...
 *  1.29 设置云端自定义数据，默认为""
 *
 *  此自定义字段会同步到 Server，用户卸载应用或则更换终端后还能继续获取。
 *
 *  @param data 设置参数
 *
 *  @return TRUE：设置成功；FALSE:设置失败
 */
- (BOOL)setCloudCustomData:(NSData*)data;

/**
 *  1.30 获取云端 CustomData
 *
 *  @return CustomData
 */
...
/**
 *  1.31 拷贝消息中的属性（ELem、priority、online、offlinePushInfo）
 *
 *  @param srcMsg 源消息
 *
 *  @return 0 成功
