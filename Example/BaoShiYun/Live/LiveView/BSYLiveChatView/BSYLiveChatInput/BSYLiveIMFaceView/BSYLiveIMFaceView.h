...
@class BSYLiveIMFaceCellData;

@protocol BSYLiveIMFaceViewDelegate <NSObject>
 ...
 *  存储各个表情的 cellData
 */
@property (nonatomic, strong) NSMutableArray<BSYLiveIMFaceCellData *> *faces;
...
@interface BSYLiveIMFaceView : UIView
...
/**
 *  委托变量，被委托者
 *  需要实现 HKYunLiveIMFaceViewDelegate 协议中要求的功能。
 */
@property (nonatomic, weak) id<BSYLiveIMFaceViewDelegate> delegate;
...
/**
 *  设置数据。
 *  用来进行 BSYLiveIMFaceView 的初始化或在需要时更新 faceView 中的数据。
 *
 *  @param data 需要设置的数据（BSYLiveIMFaceGroup）。在此 NSMutableArray 中存放的对象为 BSYLiveIMFaceGroup，即表情组。
 */
